...
function SoccerPitch({ 
  ...
  return (
    <div 
      className={`relative w-full h-32 bg-emerald-850 rounded border border-emerald-700/85 overflow-hidden ${
    ...
    >
      ...
      <div 
        className="absolute w-4 h-4 -ml-2 -mt-2 bg-amber-400 border-2 border-slate-900 rounded-full shadow-md shadow-amber-500/80 flex items-center justify-center transition-all duration-300"
        style={{ left: `${x}%`, top: `${y}%` }}
      >
        <span className="w-1.5 h-1.5 bg-red-600 rounded-full animate-ping absolute" />
        <span className="w-1 h-1 bg-red-650 rounded-full" />
      </div>

      {onChange && (
        <span className="absolute bottom-1 right-2 text-[8px] text-white/50 font-mono">
          EDIT_CLIC (X:{x}, Y:{y})
        </span>
      )}
    </div>
